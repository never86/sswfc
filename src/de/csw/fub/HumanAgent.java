...
public class HumanAgent {
	...
	public List getAllCompletedTasks() {
		...
		for (int i = 0; i < elements.size(); i++) {
			Element ele = (Element) elements.get(i);
			if (ele.elementText("state") != null
					&& ele.elementText("state").equalsIgnoreCase("1")) {
				tasks.add(parseElement(ele));
			}
		}
		return tasks;
	...
	public List getAllIncompletedTasks() {
		...
		for (int i = 0; i < elements.size(); i++) {
			Element ele = (Element) elements.get(i);
			if (ele.elementText("state") != null
					&& ele.elementText("state").equalsIgnoreCase("0")) {
				tasks.add(parseElement(ele));
			}
		}
		return tasks;
	...
	private Task parseElement(Element ele) {
		Task task = new Task();
		try {
			task.setReceivedTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
					.parse(ele.elementText("receivedTime")));
			task.setResolvedTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss")
					.parse(ele.elementText("resolvedTime")));
